...
def summary_str(num_actors, epoch, num_messages, avg_msg, min_drate, max_drate):
    return "Total actors: {}\n".format(num_actors) +\
...
def compute_simulation_statistics(sim_log, filter_func=None,\
                                  summary=True, display=True):
    """
    Given the simulation log file, compute the following
    ...
    Params:
    -------
    sim_log:      the path to the simulation log file
    filter_func:  a function used to filter messages based on
                  their timestamp value
    summary:      if true print on the console info on the
                  simulation results
    display:      a boolean assuming value true if it required
                  to print plots and statistics when calling the function

    Return:
    -------
    The delays of messages normalised on the total number of messages.
    """
    if filter_func is None:
        filter_func = lambda x: False

    actors, broadcast, msg_delivery, delivery_order, delta_msg = parse_logs(sim_log, filter_func)
    ...
    delays_normalized = da.sum_delivery_fraction(delays2)
    if summary is True:
        print(summary_str(num_actors, epoch, num_messages, avg_msg,\
                      min_drate, max_drate))
    if display is True:
        # delivery rate
        da.plt.figure()
        bins = da.plot_drate_pdf(delivery_rate)
        da.plt.xlabel("Delivery rate")
        da.plt.ylabel("pdf")
        da.plt.show()
        # delay cdf
        da.plt.figure()
        da.plot_count(delays_normalized)
        da.plt.xlabel("Delivery time (ms)")
...
if __name__ == "__main__":
    ...
    actors_folder_regexp = re.compile(".*run_(\d+)_(\d+)")
    # for each different number of actors, store
    # a list containing the folders with simulation results
    actors_folders = dict()
    folders.sort()
    for folder in folders:
        for sim_match in actors_folder_regexp.finditer(folder):
            num_actors, epto_c = sim_match.groups()
            try:
                actors_folders[num_actors].append((folder, epto_c))
            except KeyError:
                actors_folders[num_actors] = [(folder, epto_c)]
    linestyles = ["-", "--", "-.", ":"]
    # store legend elements to display on the plot
    legend = []
    for num_actors, folders_data in actors_folders.items():
        index = 0
        da.plt.figure()
        for subfolder, epto_c in folders_data:
            # do the average simulation
            simulation_logs = glob.glob(subfolder +\
                                        os.sep + "*.log")
            simulation_dict = {}
            for log in simulation_logs:
                ...
                for ts, percentage in delays.items():
                    try:
                        ...
                        simulation_dict[ts] = percentage
            # normalise based on simulation numbers
            simulation_dict = { k:v/len(simulation_logs)\
                                for k,v in simulation_dict.items() }
            da.plot_count(simulation_dict, linestyle=linestyles[index])
            legend.append("c = {}".format(epto_c))
            index += 1
